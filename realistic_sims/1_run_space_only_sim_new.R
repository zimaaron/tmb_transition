...
alphaj.pri      <- eval(parse(text = "c(0, 3)")) ## TODO normal mean and sd
nug.pri         <- eval(parse(text = "c(1, 1e-5)")) ## TODO gamma for nug preciion with shape and inv-scale
...
d <- data.table(truth        = rep(values(true.rast), 2),
                truth.p      = logit(rep(values(true.rast), 2)),
                model        = c(rep('tmb', nrow(pred_tmb)),
                                 rep('inla', nrow(pred_inla))), 
                median.fit   = c(values(ras_med_tmb),
                                 values(ras_med_inla)),
